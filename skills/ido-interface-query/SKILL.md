...
统一判断用户是在装配环境还是查询业务数据。本 Skill 已内置环境装配、统计查询和用户操作三组能力；业务查询通过 `getui-cli` 转换为个推 IDO 固定 operation，不直接访问个推 HTTP API。

## 能力分流

- **环境装配**：安装、更新、卸载、修复或验证 CLI，以及 profile 和凭证配置；环境未就绪时先按安装 reference 完成装配，不构造业务 operation。
...
- 安装、更新、卸载、修复或验证 CLI/Skill 环境时，读取 [references/environment-setup.md](references/environment-setup.md)。
...
1. 先识别环境装配或业务查询。环境装配请求读取 `environment-setup.md` 并在本 Skill 内完成；业务查询才继续选择 operation。
...
7. 失败时保留 CLI 错误码、类型、阶段、`retryable` 和脱敏后的远端信息；命令缺失或环境未配置时读取环境装配 reference 并修复，最终鉴权失败时才使用 `getui-cli app status` 辅助诊断。
