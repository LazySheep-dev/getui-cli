...
用户：“查询最近 7 天新增用户趋势。”（当前日期为 `2026-08-13`）

```bash
getui-cli api call statistics.userTrend --input '{"startDate":"2026-08-06","endDate":"2026-08-12","metric":"new"}'
```

用户：“查询最近 7 天新增留存。”（当前日期为 `2026-08-13`）

```bash
getui-cli api call statistics.retention --input '{"startDate":"2026-08-05","endDate":"2026-08-11","metric":"new"}'
```

若用户要求截止昨天的留存，说明昨天的次日留存尚未完整形成；若范围包含 31 天，直接拒绝，不生成 CLI。
...
说明这是统计请求，路由到统一 Skill 的 `statistics.*` operation，不调用用户 operation。
