...
只允许调用下表中的 21 个已注册 operation。不要根据用户输入构造 URL、HTTP 方法、请求头或未列出的 operation。默认入口是：
...
公共字段：`activityScope` 为 `foreground|all`；`groupBy` 为 `platform|channel|version|package`；维度过滤使用 `channels`、`appVersions`、`packageNames`、`platforms` 字符串数组。省略 `activityScope` 时采用接口默认前台口径。
...
`metric` 为 `new|active|start`；`groupValue` 只有在同时提供 `groupBy` 时才允许。
...
`metric` 为 `new|active|start|total|avgDuration|avgFrequency`，日期格式为 `YYYY-MM-DD`。

```bash
getui-cli api call statistics.userTrend --input '{"startDate":"2026-08-05","endDate":"2026-08-11","metric":"new"}'
```

### `statistics.retention`

`metric` 为 `new|active|start`；未指定留存类型时补充 `new`。调用前拒绝超过 30 个自然日的范围，以及结束日期晚于上海时区前天的范围。
