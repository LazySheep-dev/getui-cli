...
| Skill | 能力 |
| --- | --- |
| `ido-interface-query` | 唯一需要安装的主 Skill，内置环境装配、统计、标签、用户、用户群和向量能力 |

仓库中的另外三个目录是开发时的子 Skill 源目录，不需要单独下载或安装。
...
3. 安装依赖并构建 CLI
4. 注册 `getui-cli` 命令
5. 只安装 `skills/ido-interface-query/` 这一个完整主 Skill
...
- `ido-interface-query/SKILL.md` 及其全部 `references/` 文件存在且非空
